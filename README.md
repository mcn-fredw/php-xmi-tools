...
- Code generator makes an attempt to merge code
from pre-existing modules using PHP reflection.
...
Intended use is to pass information to specialty builders
(i.e. DoctrineEntityBuilder).
...
- Code added to an interface method will be emitted in any directly
implementing traits or classes.
...
the code generator to create a test class for the class/trait.
    ...
    - The test class module will include a stub socket class to ```extend```
...
will cause the source code generator to look for special annotations and attribute
documentation to generate doctrine YAML metadata files.
    ...
    is where to put general yaml entries for the entity.
    - Attributes in the class, or related traits can specify field yaml by
    adding a **@doctrine-yaml** section in the
    attribute documentation (like **@code** sections).
    Any text between **@doctrine-yaml** lines is emitted in the fields section
    of the entity metadata.
